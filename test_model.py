...
def construct_prompt(question: str) -> str:
    """
    Construct the prompt by appending the question with an 'Answer:' directive.
...
def perform_inference(model, tokenizer, prompts: List[str], max_gen_len: int = 700) -> List[str]:
    """
...
def main():
    ...
    # Model configuration
    model_name = "Qwen/Qwen2.5-Math-1.5B-Instruct"  # Hugging Face model name; you can choose another if desired
    ...
    # Assign pad_token to eos_token if pad_token is not set
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
        print("Pad token not found. Setting pad_token to eos_token.")
    ...
    prompts = [construct_prompt(q) for q in test_questions]
